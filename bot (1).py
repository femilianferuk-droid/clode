...
def cache_message(msg: Message, business_connection_id: str):
    ...
    from_name = (
        (msg.from_user.full_name or "") if msg.from_user else ""
    )
    media_type = None
    file_id = None
    if msg.photo:
...
def get_cached_messages(business_connection_id: str, chat_id: int, message_ids: list[int]):
    with db() as con:
...
def toggle_trigger(trigger_id: int, owner_id: int) -> bool | None:
    with db() as con:
        row = con.execute(
...
def get_business_connections(owner_id: int) -> list:
    """Return cached business connection ids seen for this owner."""
    with db() as con:
        return con.execute(
            "SELECT DISTINCT business_connection_id FROM message_cache LIMIT 50"
        ).fetchall()
...
async def cmd_menu(message: Message):
    if not is_owner(message.from_user.id):
        add_owner(message.from_user.id)
    await message.answer("🏠 <b>Главное меню</b>", parse_mode=ParseMode.HTML, reply_markup=main_menu_kb())


# ─── Callback: main menu ─────────────────────────────────────────────────────
@router.callback_query(F.data == "main_menu")
async def cb_main_menu(call: CallbackQuery):
    await call.message.edit_text("🏠 <b>Главное меню</b>", parse_mode=ParseMode.HTML, reply_markup=main_menu_kb())
...
async def cb_trigger_info(call: CallbackQuery):
    ...
    with db() as con:
        t = con.execute(
            "SELECT * FROM triggers WHERE id=? AND owner_id=?", (trigger_id, uid)
        ).fetchone()
    if not t:
...
async def cb_trigger_toggle(call: CallbackQuery):
    ...
    if result is None:
        await call.answer("Триггер не найден", show_alert=True)
        return
    state_text = "включён ✅" if result else "отключён ❌"
    await call.answer(f"Триггер {state_text}")
    # Refresh info view
    with db() as con:
        t = con.execute(
...
# ─── Callback: add trigger (FSM entry) ───────────────────────────────────────
...
async def fsm_got_keyword(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<i>Например: <code>&lt;b&gt;Наш прайс&lt;/b&gt;: example.com/price</code></i>\n\n"
        "Напиши /cancel для отмены.",
        parse_mode=ParseMode.HTML,
    )
...
# ─── Business: incoming messages (cache + auto-reply) ────────────────────────
@router.business_message()
async def on_business_message(message: Message, bot: Bot):
    """Cache every incoming business message and check triggers."""
    ...
    # Get business connection to find the owner
    try:
    ...
    triggers = get_active_triggers(owner_id)
    for t in triggers:
        if t["keyword"].lower() in text:
            try:
                ...
                logger.info("Auto-replied with trigger #%d to chat %d", t["id"], message.chat.id)
            except Exception as e:
                logger.error("Failed to auto-reply: %s", e)
            break  # one trigger per message


# ─── Business: edited messages (re-cache) ────────────────────────────────────
...
async def on_business_deleted(event: BusinessMessagesDeleted, bot: Bot):
    ...
    for row in cached:
        ...
        if row["text"]:
            ...
            content_lines.append(f"\n📝 <b>Подпись:</b>\n{row['caption']}")
        if row["media_type"]:
            content_lines.append(f"📎 <b>Медиа:</b> {row['media_type']}")

        msg_text = "\n".join(content_lines)

        try:
            # Try to resend media if available
            if row["file_id"] and row["media_type"]:
                mt = row["media_type"]
                caption = msg_text
                if mt == "photo":
                    await bot.send_photo(owner_id, row["file_id"], caption=caption, parse_mode=ParseMode.HTML)
                elif mt == "video":
                    await bot.send_video(owner_id, row["file_id"], caption=caption, parse_mode=ParseMode.HTML)
                elif mt == "document":
                    await bot.send_document(owner_id, row["file_id"], caption=caption, parse_mode=ParseMode.HTML)
                elif mt == "audio":
                    await bot.send_audio(owner_id, row["file_id"], caption=caption, parse_mode=ParseMode.HTML)
                elif mt == "voice":
                    await bot.send_voice(owner_id, row["file_id"], caption=caption, parse_mode=ParseMode.HTML)
                elif mt == "sticker":
                    await bot.send_sticker(owner_id, row["file_id"])
                    await bot.send_message(owner_id, msg_text, parse_mode=ParseMode.HTML)
                elif mt == "animation":
                    await bot.send_animation(owner_id, row["file_id"], caption=caption, parse_mode=ParseMode.HTML)
                else:
                    await bot.send_message(owner_id, msg_text, parse_mode=ParseMode.HTML)
            else:
                await bot.send_message(owner_id, msg_text, parse_mode=ParseMode.HTML)
        except Exception as e:
            logger.error("Error sending deleted message to owner: %s", e)

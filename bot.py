...
class RawUpdateMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        # Достаём сырой dict из объекта Update
        raw: dict = {}
        try:
            raw = event.model_dump(exclude_none=True)
        except AttributeError:
            try:
                raw = event.dict(exclude_none=True)
            except Exception:
                pass

        deleted = raw.get("business_messages_deleted")
        if deleted:
...
async def _process_deleted(deleted: dict, bot: Bot):
    ...
    for row in cached:
        ...
        try:
            if fid and mt == "photo":
                await bot.send_photo(owner_id, fid, caption=body, parse_mode=ParseMode.HTML)
            elif fid and mt == "video":
                await bot.send_video(owner_id, fid, caption=body, parse_mode=ParseMode.HTML)
            elif fid and mt == "document":
                await bot.send_document(owner_id, fid, caption=body, parse_mode=ParseMode.HTML)
            elif fid and mt == "audio":
                await bot.send_audio(owner_id, fid, caption=body, parse_mode=ParseMode.HTML)
            elif fid and mt == "voice":
                await bot.send_voice(owner_id, fid, caption=body, parse_mode=ParseMode.HTML)
            elif fid and mt == "sticker":
                await bot.send_sticker(owner_id, fid)
                await bot.send_message(owner_id, body, parse_mode=ParseMode.HTML)
            elif fid and mt == "animation":
                await bot.send_animation(owner_id, fid, caption=body, parse_mode=ParseMode.HTML)
            else:
                await bot.send_message(owner_id, body, parse_mode=ParseMode.HTML)
        except Exception as e:
...
async def cmd_menu(msg: Message):
    ensure_owner(msg.from_user.id)
    await msg.answer("🏠 <b>Главное меню</b>", parse_mode=ParseMode.HTML, reply_markup=main_kb())

# ── main_menu callback ─────────────────────────────────────
@router.callback_query(F.data == "main_menu")
async def cb_main(call: CallbackQuery):
    await call.message.edit_text("🏠 <b>Главное меню</b>", parse_mode=ParseMode.HTML, reply_markup=main_kb())
...
async def fsm_sched(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.answer("Неверный формат. Пример: <code>09:00-18:00</code>",
...
async def on_biz_msg(msg: Message, bot: Bot):
    ...
    fn = msg.from_user.full_name if msg.from_user else ""
    ...
    try:
        ...
        logger.warning("get_business_connection: %s", e); return
...
async def main():
    ...
    # Перехватываем business_messages_deleted через raw middleware
